...
// declare
    ...
    VectorXd timeAllocation(MatrixXd waypoints);
    void trajGeneration(MatrixXd waypoints);
...
void trajGeneration(Eigen::MatrixXd waypoints)
{
    ...
    MatrixXd derivative_cons = MatrixXd::Zero(2*_dev_order, 3);
    derivative_cons << waypoints.row(0),
                       ...
                       vel.row(1),
                       acc.row(1);
    cout << "derivative_cons = " << derivative_cons << endl;

    // give an arbitraty time allocation, all set all durations as 1 in the commented function.
    _polyTime  = timeAllocation(waypoints);

    MinimumSnapSolver minimum_snap_solver(_dev_order, waypoints, derivative_cons, _polyTime);
    // generate a minimum-snap piecewise monomial polynomial-based trajectory
    _polyCoeff = trajectoryGeneratorWaypoint.PolyQPGeneration(_dev_order, waypoints, vel, acc, _polyTime);
    _polyCoeff = minimum_snap_solver.QPSolver();
...
{ 
    ...
    float time_to_max_v = _Acc / _Vel;
    float dist_to_max_v = 0.5 * _Acc * pow(time_to_max_v, 2); 
    for (int k = 0; k < waypoints.rows()-1; k++) {
        float dist = (waypoints.row(k) - waypoints.row(k+1)).norm();
        if (dist > dist_to_max_v*2) {
            time(k) = 2*time_to_max_v + (dist - 2*dist_to_max_v)/_Vel;
        }
        else {
            time(k) = 2*sqrt(dist/_Acc);
        }
    }    
    return time;
}